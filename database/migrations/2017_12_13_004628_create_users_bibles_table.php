...
$table->timestamp('updated_at')->default(DB::raw('CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP'));
        ...
        if (!Schema::connection('dbp_users')->hasTable('project_oauth_providers')) {
            Schema::connection('dbp_users')->create('project_oauth_providers', function (Blueprint $table) {
                ...
                $table->foreign('project_id')->references('id')->on(config('database.connections.dbp_users.database').'.projects')->onDelete('cascade')->onUpdate('cascade');
                $table->string('name');
...
$table->timestamp('updated_at')->default(DB::raw('CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP'));
        ...
        if (!Schema::connection('dbp_users')->hasTable('project_members')) {
            Schema::connection('dbp_users')->create('project_members', function (Blueprint $table) {
                ...
                $table->foreign('role_id')->references('id')->on(config('database.connections.dbp_users.database').'.roles')->onDelete('cascade')->onUpdate('cascade');
...
$table->timestamp('updated_at')->default(DB::raw('CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP'));
        ...
        if (!Schema::connection('dbp_users')->hasTable('user_notes')) {
            Schema::connection('dbp_users')->create('user_notes', function (Blueprint $table) {
                $table->increments('id');
                $table->integer('user_id')->unsigned();
                $table->foreign('user_id')->references('id')->on(config('database.connections.dbp_users.database').'.users')->onUpdate('cascade')->onDelete('cascade');
...
$table->timestamp('updated_at')->default(DB::raw('CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP'));
        ...
        if (!Schema::connection('dbp_users')->hasTable('user_highlights')) {
            Schema::connection('dbp_users')->create('user_highlights', function (Blueprint $table) {
                ...
                $table->foreign('highlighted_color')->references('id')->on(config('database.connections.dbp_users.database').'.user_highlight_colors')->onDelete('cascade')->onUpdate('cascade');
                $table->timestamp('created_at')->default(DB::raw('CURRENT_TIMESTAMP'));
$table->timestamp('updated_at')->default(DB::raw('CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP'));
            });
        }
        if (!Schema::connection('dbp_users')->hasTable('user_annotation_tags')) {
            Schema::connection('dbp_users')->create('user_annotation_tags', function (Blueprint $table) {
                ...
                $table->foreign('note_id')->references('id')->on(config('database.connections.dbp_users.database').'.user_notes')->onUpdate('cascade')->onDelete('cascade');
                $table->integer('bookmark_id')->unsigned()->nullable();
                $table->foreign('bookmark_id')->references('id')->on(config('database.connections.dbp_users.database').'.user_bookmarks')->onUpdate('cascade')->onDelete('cascade');
                $table->integer('highlight_id')->unsigned()->nullable();
                $table->foreign('highlight_id')->references('id')->on(config('database.connections.dbp_users.database').'.user_highlights')->onUpdate('cascade')->onDelete('cascade');
...
$table->timestamp('updated_at')->default(DB::raw('CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP'));
        ...
        if (!Schema::connection('dbp')->hasTable('access_types')) {
            Schema::connection('dbp')->create('access_types', function (Blueprint $table) {
                ...
                $table->foreign('continent_id')->references('continent')->on(config('database.connections.dbp.database').'.countries')->onUpdate('cascade');
...
$table->timestamp('updated_at')->default(DB::raw('CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP'));
        ...
        if (!Schema::connection('dbp')->hasTable('access_group_types')) {
            Schema::connection('dbp')->create('access_group_types', function (Blueprint $table) {
                ...
                $table->foreign('access_group_id')->references('id')->on(config('database.connections.dbp.database').'.access_groups')->onUpdate('cascade')->onDelete('cascade');
                $table->integer('access_type_id')->unsigned();
                $table->foreign('access_type_id')->references('id')->on(config('database.connections.dbp.database').'.access_types')->onUpdate('cascade')->onDelete('cascade');
                $table->timestamp('created_at')->default(DB::raw('CURRENT_TIMESTAMP'));
$table->timestamp('updated_at')->default(DB::raw('CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP'));
            });
        }
...
$table->timestamp('updated_at')->default(DB::raw('CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP'));
        ...
        if (!Schema::connection('dbp_users')->hasTable('access_group_keys')) {
            Schema::connection('dbp_users')->create('access_group_keys', function (Blueprint $table) {
                ...
                $table->foreign('key_id')->references('key')->on(config('database.connections.dbp_users.database').'.user_keys')->onUpdate('cascade')->onDelete('cascade');
                $table->timestamp('created_at')->default(DB::raw('CURRENT_TIMESTAMP'));
$table->timestamp('updated_at')->default(DB::raw('CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP'));
            });
        }
