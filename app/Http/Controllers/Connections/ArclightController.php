...
class ArclightController extends APIController
{
	...
	private function fetchMediaForLanguage($iso)
    {
    	...
    	if(!file_exists(storage_path("/data/jfm/languages/$iso.json"))) {
			...
			$media_components = $this->fetch('media-components', ['languageIds' => $language_id,'subTypes' => 'featureFilm']);
			$media_components = collect( $media_components->mediaComponents )->pluck( 'mediaComponentId' );
			file_put_contents( storage_path( "/data/jfm/languages/$iso.json" ), json_encode( $media_components, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE ) );
		}
		else {
	    ...
	    foreach ($media_components as $media_component) $feature_films[$media_component] = $this->fetch("media-components/$media_component/languages/$language_id");
    ...
    private function fetch($path,$params = [])
    {
	    ...
	    if(isset($results->_embedded)) return $results->_embedded;
	    return $results;
    }
