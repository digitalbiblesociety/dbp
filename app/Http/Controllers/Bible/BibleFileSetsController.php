...
class BibleFileSetsController extends APIController
{
	...
	public function show($id = null)
	{
		...
		if(!$fileset) return $this->setStatusCode(404)->replyWithError(trans('api.bible_fileset_errors_404_asset', ['asset_id' => $asset_id]));
	...
	public function download($id)
	{
		...
		if ($books) {
			...
			if(!$files) return $this->setStatusCode(404)->replyWithError('Files not found');
	...
	public function copyright($id)
	{
		...
		if(!$language) return $this->setStatusCode(400)->replyWithError(trans('api.languages_errors_404'));
