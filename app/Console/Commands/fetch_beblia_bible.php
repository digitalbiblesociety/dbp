...
class fetch_beblia_bible extends Command
{
    ...
    public function handle()
    {
	    ...
	    if(!file_exists(storage_path('bibles/'.$name))) mkdir(storage_path('bibles/'.$name));
	    ...
	    while($book_number <= $totalBooks) {
		    ...
		    while($chapter_number <= $totalChaptersCount) {
			    ...
			    foreach($rows as $i => $row) {
				    if($row->find('.verseTextButton')) $chapter .= '\v '.$row->find('.verseTextButton')[0]->innertext().' ';
				    if($row->find('.verseTextText'))   $chapter .= $row->find('.verseTextText')[0]->innertext()." \n";
			    }
