...
class sync_scriptureEarth extends Command
{
    ...
    public function handle()
    {
	    ...
	    foreach($language_rows as $language_row) {
		    ...
		    if(!isset($items[2]->plaintext)) { echo "Missing: ".$items[2]->plaintext; continue; }
		    if(!isset($language)) { echo "Not Found: ".$items[2]->plaintext; continue; }
		    ...
		    foreach($language_page_items as $language_page_item) {
			    ...
			    if(str_contains($link_text,['JESUS Film','JESUSFilm','Magdalena','Check SIL.org',"href='#'"])) { continue; }
			    if(!str_contains($link_text, 'href')) { continue; }
			    ...
			    if($current_language) file_put_contents(storage_path("data/ScriptureEarth/". $items[2]->plaintext .'.json'), $current_language, FILE_APPEND);
		    }
