...
class SyncUsers extends Command
{
    ...
    public function syncAccounts()
    {
        ...
        foreach ($accounts as $account) {
            ...
            if (!$user) {
                $missingUsers[] = $account->user_id;
                continue;
            }
            if (Account::where(['user_id' => $user->id,'provider_id' => $account->remote_type])->exists()) {
                continue;
            }
            Account::create([
                'user_id'          => $user->id,
    ...
    public function syncNotes()
    {
        ...
        foreach ($filesets as $fileset) {
            if ($fileset->bible->first()) {
                $bibles[$fileset->id] = $fileset->bible->first()->id;
            }
        }
            ...
            \DB::connection('dbp_users_v2')->table('note')->orderBy('created', 'asc')
                                           ...
                                           ->chunk(50000, function ($new_notes) use ($books, $bibles) {
                                            foreach ($new_notes as $note) {
                                                $bible_id = false;
                                                if (isset($bibles[$note->dam_id])) {
                                                    $bible_id = $bibles[$note->dam_id];
                                                }
                                                if (isset($bibles[substr($note->dam_id, 0, -4)])) {
                                                    if (!$bible_id) {
                                                        $bible_id = $bibles[substr($note->dam_id, 0, -4)];
                                                    }
                                                }
                                                if (isset($bibles[substr($note->dam_id, 0, 6)])) {
                                                    if (!$bible_id) {
                                                        $bible_id = $bibles[substr($note->dam_id, 0, 6)];
                                                    }
                                                }
