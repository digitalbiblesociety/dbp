...
class organizations_dbl_status extends Command
{
    ...
    public function handle()
    {
	    ...
	    switch ($arguments['action']) {
		    case "compare": {$this->compareOrganizations(); break;}
		    case "update":  {$this->updateOrganizations();  break;}
	    }
    }
    ...
    public function compareOrganizations()
    {
	    ...
	    foreach($organizations->orgs as $dbl_organization) {
		    ...
		    if($relationshipAlreadyExists OR ($dbl_id == $dbl_organization->id)) {continue;}
		    ...
		    if($translationMatchExists) {
			    ...
			    if ($organizations->count() == 0) { $missing[] = $dbl_organization->full_name; continue; }
