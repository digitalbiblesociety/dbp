...
class update_bible_links extends Command
{
    ...
    public function organizationFocus()
    {
	    ...
	    foreach ($bible_links as $bible_link) {
		    ...
		    if(in_array($bible_link->provider,$skippedProviders)) {continue;}
		    ...
		    while($confirmed == false) {
			    ...
			    if(!in_array($organization_id,$organizations->pluck('organization_id')->toArray())) {
				    if($this->confirm("Your selection $organization_id is not in the recommendations, is that your intention? [yes|no]")) $confirmed = true;
		        }
