...
<section role="banner" itemscope itemtype="http://schema.org/WPHeader"  @if(!isset($noGradient) and isset($backgroundImage)) class="darken" @endif>
    ...
    @if(isset($title))
        <div class="banner-heading">
            <h1 itemprop="headline" @if(isset($title_direction)) dir="{{ $title_direction }}" @endif @if(isset($title_class)) class="{{ $title_class }}" @endif>{{ $title ?? "" }}</h1>
                ...
                @elseif($value != "")
                    <a role="tab"
                       @if(!key_exists("#",$tabs))
                            ...
                            @else @if($key == "#") aria-selected="true" @else aria-selected="false"  @endif
                       @endif
